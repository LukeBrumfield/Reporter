(function() {
  ...
  callWithJQuery(function($) {
    ...
    pivotTableRenderer = function(pivotData, opts) {
      var append, aggregator, c, colAttrs, colKey, colKeys, defaults, i, j, r, result, rowAttrs, rowKey, rowKeys, spanSize, tbody, thead, td, th, totalAggregator, tr, txt, val, x;
      defaults = {
        localeStrings: {
          totals: "Totals"
        }
      ...
      //first row has row element label, highest order col labels, total
      tr = document.createElement("tr");

      //add topleft corner, row element label
      ...
      //td.setAttribute("colspan", rowAttrs.length + (colAttrs.length === 0 ? 0 : 1));
      tr.appendChild(td);
      for (j in colKeys) {
        if (!hasProp.call(colKeys, j)) continue;
        colKey = colKeys[j];
